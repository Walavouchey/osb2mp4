...
namespace sb {
    ...
    void Storyboard::generateAudio(const std::string &outFile) const
    {
    ...
    cv::Mat Storyboard::DrawFrame(double time)
    {
    ...
    cv::Mat Storyboard::GetVideoImage(double time)
    {
        int offset = 500;
...
#pragma omp critical
    ...
    void Storyboard::RasteriseQuad(cv::MatIterator_<cv::Vec<uint8_t, 3>> frameStart, cv::MatConstIterator_<cv::Vec<float, 4>> imageStart, int imageWidth, int imageHeight, cv::Point2f quad[4], Colour colour, bool additive, double alpha) const
    {
        alpha /= 255.0;
    ...
    void Storyboard::SampleColourAndAlpha(cv::MatConstIterator_<cv::Vec<float, 4>> imageStart, int width, int height, float u, float v, Colour& outputColour, float& outputAlpha) const
    {
    ...
    void Storyboard::ScanLine(int x1, int y1, int x2, int y2, std::vector<std::pair<int, int>>& ContourX) const
    {
